...
contract('FABToken', async (accounts) => {
    ...
    describe('token attributes', async function () {
        ...
        it('Transfer 20% to exchange', async function () {
            await this.token.transfer(exchangeAddress, web3.utils.toWei((_totalSupply * 20 / 100).toString(), 'ether'))
        });
        

        it('Total Supply is 55 billion', async function () {
            const totalSupply = await this.token.totalSupply();
            totalSupply.toString().should.equal(`${web3.utils.toWei(_totalSupply.toString(), 'ether')}`);
        });
        ...
        it('Owner has 80% of 55 billion balance , i.e total supply', async function () {
            ...
            balanceOfOwner.toString().should.equal(`${web3.utils.toWei((_totalSupply * 80 / 100).toString(), 'ether')}`);
        });
        ...
        it('Exchange can freely transfer 100 to customer 1', async function () {
            var tokens = 100
            await this.token.transfer(fabCustomer1, web3.utils.toWei(tokens.toString(), 'ether'), { from: exchangeAddress });
        });
        ...
        it('Balance of customer 1 should be 100', async function () {
            ...
            var tokens = 100
            balance.toString().should.equal(web3.utils.toWei(tokens.toString(), 'ether'));
        });
        ...
        it('Owner transfer 100 to customer 1', async function () {
            var tokens = 100
            const status = await this.token.transfer(fabCustomer1, web3.utils.toWei(tokens.toString(), 'ether'), {from: owner});
        });
        ...
        it('Balance of owner should be 44 billion - 100', async function () {
            ...
            let tokens = _totalSupply * 80 / 100 - 100;
            balance.toString().should.equal(`${web3.utils.toWei(tokens.toString(), 'ether')}`);
        });
        
        it('Customer 1 tries to transfer more token than his balance to customer 2, he fails to do so', async function () {
            var tokens = 201
            await expectThrow(this.token.transfer(fabCustomer2, web3.utils.toWei(tokens.toString(), 'ether'), { from: fabCustomer1 }));
        });
    });
});
