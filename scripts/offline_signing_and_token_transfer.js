const main = async () => {
    ...
    const rawData = fs.readFileSync('build/contracts/FABToken.json')
    ...
    const TOKEN_TIMELOCK_ADDRESS = '0xB772c21C3832DD97819Fe52B8cFa4ae592B296Bd'.toLowerCase() // Address of the deployed Token timelock contract
    const OWNER_BALANCE = (await erc20.methods.balanceOf(OWNER_ADDRESS).call()) // Do not modify
    const BALANCE_TO_TRANSFER = web3.utils.toWei((TOTAL_SUPPLY * 20 / 100).toString(), 'ether') // Number of tokens you wish to transfer to receiving address
    ...
    var rawTransaction = {
        ...
        value: 0,
        data: erc20.methods.transfer(TOKEN_TIMELOCK_ADDRESS, BALANCE_TO_TRANSFER.toString()).encodeABI(),
